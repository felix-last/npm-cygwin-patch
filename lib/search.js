...
function search (args, silent, staleness, cb) {
  ...
  updateIndex(staleness, function (er, allEntriesStream) {
    if (er) return cb(er)
    searchInStream(allEntriesStream, filter, args, opts, searchexclude, silent, cb)
  })
}

function searchInStream (stream, filter, args, opts, exclude, silent, cb) {
  log.silly('search', 'searching packages')

  var filterStream = createFilterStream(filter, opts, exclude, silent)
  ...
  ms.pipe(stream, filterStream, outputStream, function (er) {
    if (er) return cb(er)
...
function createFilterStream (filter, opts, exclude, silent) {
  ...
  return ms.through.obj(function (pkg, enc, cb) {
    ...
    if (!silent && filter(pkg, opts, exclude)) {
      cb(null, pkg)
    } else {
      cb()
    }
