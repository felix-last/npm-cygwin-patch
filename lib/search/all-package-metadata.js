...
function createCacheEntryStream (cacheFile, cb) {
  log.verbose('update-index', 'creating entry stream from local cache')
  log.verbose('update-index', cacheFile)
...
function createEntryUpdateStream (all, auth, staleness, latest, cb) {
  log.verbose('update-index', 'creating remote entry stream')
  var params = {
    timeout: 600,
    follow: true,
    staleOk: true,
