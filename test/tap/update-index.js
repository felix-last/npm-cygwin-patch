...
test('updateIndex cache only', function (t) {
  setup()
  var now = Date.now()
  var cacheContents = {
    '_updated': now,
  ...
  // TODO - figure out how to get the time stuff in here
  updateIndex(10000000, function (err, stream) {
    if (err) throw err
    t.ok(stream, 'returned a stream')
    var results = []
    stream.on('data', function (pkg) {
...
test('updateIndex no sources', function (t) {
  setup()
